...
const updateTime = () => {
    Object.assign(barRef.value.style, {
        width: `${(videoRef.value.currentTime * containerBar.value.clientWidth) / videoRef.value.duration
            }px`,
    })
        ...
        <BackDrop>
            <div class="relative h-full w-full sm:h-[881px] sm:w-[495.5px]" ref="mediaContainerRef">
                <div class="absolute inset-0">
                    <video @click="togglePlay" class="h-full w-full object-cover"
                        src="/video/277951101_388380176198930_7921718664685624255_n.mp4" autoplay loop muted
                        ref="videoRef" />
                </div>
                <div @click="togglePlay" ref="bigPlayIcon" :class="
                    clsx(
